...
def create_app() -> Flask:
    ...
    def scrape_now():
        # Manual refresh: replace stale per-source events so the UI reflects
        # exactly what the scrapers just returned. The scheduler will skip
        # if it fires while this is running (see services._scrape_lock).
        replace_flag = request.args.get("replace", "1").strip().lower() not in {"0", "false", "no"}
        result = run_scrape(replace=replace_flag)
        result["ran_at_utc"] = datetime.utcnow().isoformat()
        result["replace"] = replace_flag
