...
# scheduler) skip rather than queue when a manual run is already active.
_scrape_lock = threading.Lock()


def try_acquire_scrape_lock(timeout: float | None = 0) -> bool:
    if timeout is None or timeout <= 0:
        return _scrape_lock.acquire(blocking=False)
    return _scrape_lock.acquire(timeout=timeout)
...
def run_scrape(replace: bool = False) -> dict[str, Any]:
    report = run_scrape_detailed(replace=replace)
    return {
...
def run_scheduled_scrape() -> dict[str, Any]:
    """Scheduler entrypoint: skip rather than wait if a manual scrape is running."""
    if not try_acquire_scrape_lock():
        return {"skipped": True, "reason": "another_scrape_in_progress"}
    try:
        return run_scrape_detailed(replace=False, _lock_already_held=True)
    finally:
        release_scrape_lock()
...
def run_scrape_detailed(
    ...
    if not _lock_already_held:
        acquired_here = try_acquire_scrape_lock(timeout=120)
        if not acquired_here:
            return {
                ...
                "sources": [],
                "skipped": True,
                "reason": "scrape_lock_timeout",
            }

    try:
        return _run_scrape_inner(replace=replace)
    finally:
        if acquired_here:
            release_scrape_lock()
...
def _run_scrape_inner(replace: bool) -> dict[str, Any]:
    ...
    for scraper in build_scrapers():
        ...
        if replace and source_info["processed"] > 0:
            removed = _delete_stale_events_for_source(source_name, seen_external_ids)
            source_info["removed_stale"] = removed
...
def _delete_stale_events_for_source(source_name: str, keep_external_ids: set[str]) -> int:
    """Delete events for a source that weren't seen in the latest fetch."""
    if not keep_external_ids:
        return 0
    with SessionLocal() as session:
        stmt = delete(Event).where(
            Event.source_name == source_name,
            Event.external_id.notin_(keep_external_ids),
        )
