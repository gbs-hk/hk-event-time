...
scrapeButton.addEventListener("click", async () => {
  ...
  const overlayText = document.getElementById("scrapeOverlayText");
  scrapeButton.disabled = true;
  scrapeButton.innerHTML = '<span class="scrape-button-spinner" aria-hidden="true"></span>Refreshing...';
  if (overlay) {
    if (overlayText) overlayText.textContent = "Refreshing events from sources\u2026";
    overlay.hidden = false;
  }
  updateCalendarStatus("Refreshing source listings...");
  try {
    const response = await fetch("/api/scrape-now?replace=1", { method: "POST" });
    let summary = "Refresh complete.";
    if (response.ok) {
      try {
        const data = await response.json();
        if (data && data.skipped) {
          summary = "Another scrape was already running \u2014 try again in a moment.";
        } else if (data) {
          const processed = data.processed ?? 0;
          const total = data.sources_total ?? 0;
          const failed = data.failed_sources ?? 0;
          summary = `Refreshed ${processed} events from ${total} sources` + (failed ? ` (${failed} failed)` : "") + ".";
        }
      } catch (_err) {
        /* ignore parse errors, fall through */
      }
    } else {
      summary = `Refresh failed (HTTP ${response.status}).`;
    }
    updateCalendarStatus(summary);
    calendar.refetchEvents();
  } catch (err) {
    updateCalendarStatus("Refresh failed: " + (err && err.message ? err.message : "unknown error"));
  } finally {
